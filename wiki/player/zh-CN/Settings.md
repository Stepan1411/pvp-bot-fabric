...
| `目标玩家` |布尔 | - |真实 |可以针对玩家|
| `目标生物` |布尔 | - |假 |可以瞄准敌对生物 |
| `目标机器人` |布尔 | - |假 |可以针对其他机器人 |
| `批评者` |布尔 | - |真实 |执行致命一击 |
| `远程` |布尔 | - |真实 |使用弓/弩 |
| `狼牙棒` |布尔 | - |真实 |使用带有风弹的狼牙棒 |
| `矛` |布尔 | - |假 |使用矛（由于地毯错误而禁用）|
| `攻击冷却时间` |整数 | 1-40 | 1-40 10 | 10攻击之间的滴答声|
| `近战范围` |双| 2-6 | 2-6 3.5 | 3.5近战攻击距离|
| `移动速度` |双| 0.1-2.0 | 1.0 |移动速度倍增|
| `视距` |双| 5-128 | 5-128 64 | 64最大目标检测范围|
| `撤退` |布尔 | - |真实 | HP低时撤退|
| `撤退` |双| 0.1-0.9| 0.3 | 0.3开始撤退的HP百分比（30%）|
...
|设置|类型 |范围 |默认 |描述 |
|--------|------|--------|---------|------------|
| `自动药剂` |布尔 | - |真实 |自动使用治疗/增益药水 |
| `蜘蛛网` |布尔 | - |真实 |使用蜘蛛网来减缓敌人的速度 |
...
|--------|------|--------|---------|------------|
| `bhop` |布尔 | - |真实 |启用兔子跳 |
| `bhopcooldown` |整数 | 5-30 | 5-30 12 | 12 bhop 跳跃之间的滴答声 |
| ‘跳跃提升’ |双| 0.0-0.5 | 0.0 | 0.0额外跳跃高度|
| `空闲` |布尔 | - |真实 |没有目标就流浪|
| `空转半径` |双| 3-50 | 3-50 10 | 10怠速漂移半径|
...
|设置|类型 |范围 |默认 |描述 |
|--------|------|--------|---------|------------|
| `自动装甲` |布尔 | - |真实 |自动装备最好的装甲|
| `自动武器` |布尔 | - |真实 |自动装备最好的武器|
| `自图腾` |布尔 | - |真实 |副手自动装备图腾 |
| `自动屏蔽` |布尔 | - |真实 |格挡时自动使用护盾 |
| `偏好词` |布尔 | - |真实 |比起斧子更喜欢剑 |
| `破盾` |布尔 | - |真实 |切换到斧头打破敌人的盾牌 |
| `droparmor` |布尔 | - |假 |掉落更差的护甲碎片 |
| `掉落武器` |布尔 | - |假 |掉落更糟糕的武器 |
| `下降距离` |双| 1-10 | 1-10 3.0 |物品拾取距离|
| `间隔` |整数 | 1-100 | 1-100 20 |设备检查间隔（蜱）|
| `次要级别` |整数 | 0-100 | 0 |装备的最低装甲等级 |
...
|设置|类型 |范围 |默认 |描述 |
|--------|------|--------|---------|------------|
| '失误' |整数 | 0-100 | 10 | 10错过攻击的几率 (%) |
| `错误机会` |整数 | 0-100 | 5 |攻击错误方向的几率 (%) |
| `反应延迟` |整数 | 0-20 | 0 |反应前的延迟（滴答声）|
